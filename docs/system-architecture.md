...
The runtime creates a complete procedural diorama from rounded boxes, capsules, spheres, cylinders, and toruses: platform, avatar, desk, monitors, server, plant, coffee, and eight animated developer mascots. Compact screens use six mascots, a smaller composition, and a lower pixel-ratio cap. Rendering is capped at roughly 45fps on desktop and 30fps on compact screens, and it stops while offscreen or hidden. A dedicated rotation controller applies damped pointer/touch movement to the actual Three.js group, exposes deterministic rotation state for browser checks, preserves vertical touch scrolling, and accepts rotate/reset events from the HTML control strip. Reduced-motion and forced-colors modes never start the runtime.
