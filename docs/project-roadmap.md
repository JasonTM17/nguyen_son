...
The source implements a bilingual Nguyen Son student portfolio with typed selected-work/archive data, browser-local English/Vietnamese selection, anchor navigation, local motion and forced-colors handling, a high-detail local systems-studio artwork, and an optional four-icon Three.js enhancement. The repository also contains unit, content-contract, browser accessibility, lint, typecheck, and build scripts.
...
| Visual performance | Keep the 3D icon layer optional, lazy-loaded, disposable, and secondary to the primary artwork and HTML content. | Preserve the 45/30fps caps, four-icon limit, device-pixel-ratio limits, visibility pausing, fallback, and cleanup paths. |
