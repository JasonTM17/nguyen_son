...
The hero contains `StudioScene`. It always includes the project-local portrait artwork and inline SVG fallback. The Three.js runtime is dynamically imported only when the page is not in reduced-motion or forced-colors mode. It composes a full procedural avatar/desk diorama and animated computer-science mascots, with a smaller six-mascot composition on compact screens. Direct drag/swipe rotates the actual Three.js group; HTML left/reset/right controls provide the keyboard path. Runtime orchestration lives in `studio-scene-runtime.ts`; diorama, mascots, reusable primitives, and rotation behavior are separated into focused `studio-scene-*` modules; cleanup helpers remain in `three-resource-cleanup.ts`.
