...
| `plans/` | Implementation-plan material and visual QA artifacts. |

Generated `dist/`, `node_modules/`, Playwright reports, test results, and TypeScript build-info files are ignored by `.gitignore`.
...
| `src/components/` | Header, footer, hero, work, principles, motion control, SVG fallback, and Signal Lattice runtime components. |
...
| `src/styles/` | `base.css`, `layout.css`, `components.css`, `signal-lattice.css`, and `content-sections.css`. |
...
The hero contains the Signal Lattice. Its SVG is always in the component tree. The Three.js runtime is dynamically imported only when the page is not in reduced-motion or forced-colors mode. Runtime code is isolated in `signal-lattice-scene-runtime.ts`; resource disposal helpers live in `signal-lattice-resource-cleanup.ts`.
...
| --- | --- |
| `src/app.test.tsx` | Main content, repository-link count, SVG fallback, and local motion reduction. |
| `src/components/signal-lattice-scene.test.tsx` | SVG fallback behavior for initialization failure, reduced motion, unavailable `matchMedia`, and WebGL context loss. |
| `src/components/signal-lattice-resource-cleanup.test.ts` | Disposal of every completed Three.js resource. |
